        ...
        <div class="grid md:grid-cols-2 gap-4">
            ...
            <div class="bg-white p-5 rounded-2xl shadow-sm border border-amber-100 flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <div class="h-12 w-12 bg-amber-100 text-amber-700 rounded-full flex items-center justify-center font-black text-xl uppercase">
            ...
            <div class="border border-gray-100 bg-gray-50/50 p-6 rounded-2xl space-y-4">
                <div class="flex justify-between items-start">
                    <div class="flex items-center gap-3">
                        <div class="h-10 w-10 bg-indigo-600 text-white rounded-full flex items-center justify-center font-bold uppercase">
    ...
    <div class="grid lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="flex items-center justify-between">
                <h2 class="text-2xl font-bold text-gray-900">Find a Partner</h2>
                <span class="text-sm font-bold text-indigo-600 bg-indigo-50 px-4 py-1.5 rounded-lg border border-indigo-100">Suggestions</span>
            </div>
            
            @if($suggestedPartners->count() > 0)
            <div class="grid sm:grid-cols-2 gap-6">
                @foreach($suggestedPartners as $partner)
                <div class="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center text-center transition hover:shadow-lg hover:border-indigo-100">
                    ...
                    <p class="text-sm text-gray-500 font-semibold mb-6">{{ $partner->total_points }} Total Points</p>
                    
                    <form action="{{ route('partner.request', $partner->id) }}" method="POST" class="w-full">
                        @csrf
                        <button class="w-full bg-gray-50 border border-gray-200 hover:bg-indigo-600 hover:border-indigo-600 hover:text-white text-indigo-700 font-bold py-3 rounded-xl transition shadow-sm text-sm">
                            Send Request
                        </button>
                    </form>
                </div>
        ...
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                ...
                <span class="text-sm font-bold text-emerald-600 bg-emerald-50 px-4 py-1.5 rounded-lg border border-emerald-100">Top 10</span>
            </div>

            <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                <div class="space-y-3">
                    @foreach($leaderboard as $index => $boardUser)
                    <div class="flex items-center justify-between p-3 rounded-2xl {{ $boardUser->id == auth()->id() ? 'bg-indigo-50 border border-indigo-100 shadow-sm' : 'border border-transparent' }} transition">
                        <div class="flex items-center gap-3">
                            <div class="h-8 w-8 rounded-full flex items-center justify-center font-black text-xs
                                {{ $index == 0 ? 'bg-amber-400 text-white shadow-sm' : 
                                  ($index == 1 ? 'bg-gray-300 text-white shadow-sm' : 
                                  ($index == 2 ? 'bg-orange-400 text-white shadow-sm' : 'bg-gray-100 text-gray-500')) }}">
                                {{ $index + 1 }}
                            </div>
                            <div>
                                <h4 class="font-bold text-gray-900 text-xs">
                                    {{ $boardUser->name }} 
                                    @if($boardUser->id == auth()->id()) <span class="text-[10px] text-indigo-600 ml-0.5">(You)</span> @endif
                                </h4>
                                <p class="text-[10px] font-bold text-gray-400 mt-0.5">{{ $boardUser->level }}</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <span class="font-black text-indigo-600 text-sm">{{ $boardUser->total_points }}</span>
                            <span class="text-[9px] text-gray-400 block uppercase font-bold">pts</span>
                        </div>
                    </div>
                    @endforeach
                </div>
