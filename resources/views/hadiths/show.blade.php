...
<div class="max-w-4xl mx-auto py-8">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="bg-emerald-600 text-white p-5 font-bold flex justify-between items-center">
            ...
            <span class="bg-emerald-800 px-3 py-1 rounded-full text-xs font-black">{{ $subCategories->count() }} টি পরিচ্ছেদ</span>
        </div>

        <div class="divide-y divide-gray-100">
            @foreach($subCategories as $sub)
                <a href="{{ route('hadiths.chapter', $sub->id) }}" class="block hover:bg-emerald-50/50 transition p-5 group">
                    <div class="flex justify-between items-center">
                        <div class="flex items-center gap-3">
                            <svg class="w-5 h-5 text-gray-300 group-hover:text-emerald-500" fill="currentColor" viewBox="0 0 20 20"><path d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z"></path></svg>
                            <h3 class="font-bold text-gray-800 text-lg group-hover:text-emerald-700">{{ $sub->name_bn }}</h3>
                        </div>
                        <span class="text-xs font-bold text-gray-500 bg-gray-100 group-hover:bg-emerald-100 group-hover:text-emerald-700 px-3 py-1.5 rounded-lg">
    ...
    @if($uncategorizedHadiths->count() > 0)
        <div class="mt-10 space-y-8">
            <h3 class="font-bold text-gray-500 uppercase tracking-wider text-sm mb-4">Other Hadiths</h3>
